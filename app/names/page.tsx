...
export default function NamesOfAllahPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 pb-24 md:pb-12 transition-colors duration-300">
      
      {/* Header */}
      <header className="sticky top-0 z-30 bg-white/95 dark:bg-slate-900/95 backdrop-blur border-b border-slate-100 dark:border-slate-800 px-6 py-4 flex items-center justify-between shadow-sm transition-colors duration-300">
        <button 
        ...
        <div className="text-center">
          <h1 className="text-xl font-extrabold text-slate-800 dark:text-slate-100 tracking-tight transition-colors">Asma ul-Husna</h1>
          <p className="text-[10px] uppercase tracking-widest text-emerald-600 dark:text-emerald-400 font-bold transition-colors">The 99 Names of Allah</p>
        </div>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        <div className="text-center mb-10">
          <div className="w-16 h-16 bg-emerald-100 dark:bg-emerald-900/30 rounded-2xl flex items-center justify-center mx-auto mb-4 rotate-3 transition-colors">
             <Sparkles className="w-8 h-8 text-emerald-500 dark:text-emerald-400" />
          </div>
          <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto font-medium transition-colors">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
          {ALLAH_NAMES.map((name, index) => (
            <motion.button
              ...
              className="bg-white dark:bg-slate-900 p-6 rounded-3xl border-2 border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:border-emerald-200 dark:hover:border-emerald-500 transition-all flex flex-col items-center justify-center text-center group"
            >
              <span className="text-emerald-200 dark:text-emerald-500/50 text-xs font-bold mb-2 transition-colors">#{name.id}</span>
              <h2 className="text-3xl md:text-4xl font-serif font-bold text-slate-800 dark:text-slate-100 mb-2 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors" dir="rtl">
                {name.arabic}
              </h2>
              <p className="text-xs md:text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1 transition-colors">
      ...
      <AnimatePresence>
        {selectedName && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 md:p-6 transition-colors"
          >
             <motion.div 
               initial={{ scale: 0.9, y: 20, opacity: 0 }}
               animate={{ scale: 1, y: 0, opacity: 1 }}
               exit={{ scale: 0.9, y: 20, opacity: 0 }}
               className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col transition-colors"
             >
                {/* Modal Header */}
                <div className="bg-emerald-600 dark:bg-emerald-700 p-8 relative text-center flex flex-col items-center transition-colors">
                   ...
                   <span className="text-emerald-200 dark:text-emerald-300 text-sm font-bold mb-2 relative z-10 transition-colors">Name #{selectedName.id}</span>
                   ...
                   <p className="text-emerald-100 dark:text-emerald-50 font-medium tracking-wide uppercase text-sm mt-1 relative z-10 transition-colors">{selectedName.meaning}</p>
                </div>

                {/* Modal Body */}
                <div className="p-8 md:p-10 bg-slate-50 dark:bg-slate-900 flex-1 transition-colors duration-300">
                   <div className="flex items-center gap-3 mb-4">
                      <div className="w-10 h-10 bg-emerald-100 dark:bg-emerald-900/40 rounded-xl flex items-center justify-center shrink-0 transition-colors">
                         <BookOpen className="w-5 h-5 text-emerald-600 dark:text-emerald-400 transition-colors" />
                      </div>
                      <h4 className="text-lg font-bold text-slate-800 dark:text-slate-100 transition-colors">How to reflect</h4>
                   </div>
                   
                   <p className="text-slate-600 dark:text-slate-300 leading-relaxed text-lg mb-8 transition-colors">
                   ...
                   <AnimatePresence mode="wait">
                     {!hasReflected ? (
                       <motion.button
                         ...
                         className="w-full py-4 bg-slate-800 dark:bg-slate-700 text-white rounded-2xl font-bold text-lg shadow-lg hover:bg-slate-700 dark:hover:bg-slate-600 transition-all flex items-center justify-center gap-2"
                       >
                       ...
                       <motion.div
                         ...
                         className="w-full py-4 bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300 rounded-2xl font-bold text-lg border-2 border-emerald-200 dark:border-emerald-800/50 flex items-center justify-center gap-2 transition-colors"
                       >
                         <Zap className="w-5 h-5 text-yellow-500 dark:text-yellow-400 fill-current transition-colors" />
                         Awesome! +5 XP Added
                       </motion.div>
                     )}
                   </AnimatePresence>
